...
namespace ShiryuTeam::UI {
	...
	void Console::OnUIRender()
	{
		...
		strncpy(inputBuffer, m_MessageBuffer.c_str(), sizeof(inputBuffer) - 1);
		inputBuffer[sizeof(inputBuffer) - 1] = '\0';
		if (ImGui::InputText("##input", inputBuffer, input_text_flags))
		{
			m_MessageBuffer = inputBuffer;
			if (m_MessageSendCallback)
				m_MessageSendCallback(inputBuffer);
