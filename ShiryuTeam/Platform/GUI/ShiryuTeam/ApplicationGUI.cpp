...
static ShiryuTeam::Application* s_Instance = nullptr;
...
static void FrameRender(ShiryuTeam::Application* application, ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
{
	VkResult err;
...
#include "ShiryuTeam/Embed/ShiryuTeam-Icon.embed"
#include "ShiryuTeam/Embed/mac_close.embed"
#include "ShiryuTeam/Embed/mac_minimize.embed"
#include "ShiryuTeam/Embed/mac_maximize.embed"
#include "ShiryuTeam/Embed/WindowImages.embed"
	...
	void Application::Init()
	{
		...
		{
			// Use embedded icon from resource
			uint32_t w, h;
			icon.pixels = (unsigned char*)Image::Decode(g_ShiryuTeamIcon, sizeof(g_ShiryuTeamIcon), w, h);
			icon.width = w;
			icon.height = h;
		}
		...
		// Load images
		{
			...
			// App icon (titlebar logo)
			void* data = Image::Decode(g_ShiryuTeamIcon, sizeof(g_ShiryuTeamIcon), w, h);
			m_AppHeaderIcon = std::make_shared<ShiryuTeam::Image>(w, h, ImageFormat::RGBA, data);

			// macOS control buttons
			data = Image::Decode(g_mac_closeIcon, sizeof(g_mac_closeIcon), w, h);
			m_IconClose = std::make_shared<ShiryuTeam::Image>(w, h, ImageFormat::RGBA, data);

			data = Image::Decode(g_mac_minimizeIcon, sizeof(g_mac_minimizeIcon), w, h);
			m_IconMinimize = std::make_shared<ShiryuTeam::Image>(w, h, ImageFormat::RGBA, data);

			data = Image::Decode(g_mac_maximizeIcon, sizeof(g_mac_maximizeIcon), w, h);
			m_IconMaximize = std::make_shared<ShiryuTeam::Image>(w, h, ImageFormat::RGBA, data);
			m_IconRestore = std::make_shared<ShiryuTeam::Image>(w, h, ImageFormat::RGBA, data);
